...
| 💰 **Finance** | Admin-only cash ledger: money in / out by category, monthly income vs expense, parties (financers, card owners, hosts, vendors) with a running "we owe" balance, accounts (bank / cash / UPI / borrowed credit cards) with balances, **Excel / PDF report export** per month, date range or all time for the accountant |
...
- **Vitest** + **Playwright** for testing (298 unit + 317 integration + 16 E2E)
...
├── tests/
│   ├── integration/        # 22 files, 317 tests (real Postgres)
...
|---|---|---|---|
| Unit | `npm run test` | 298 tests, 9 files | `src/lib/` helpers — permissions, activity, utm, crypto, trend, env, finance, finance-report, password-reset |
| Integration | `npm run test:int` | 317 tests, 22 files | Every API route against real Postgres — auth gates, validation, persistence, audit log |
