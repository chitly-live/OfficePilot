...
- **Vitest** + **Playwright** for testing (326 unit + 347 integration + 16 E2E)
...
├── tests/
│   ├── integration/        # 28 files, 347 tests (real Postgres)
...
|---|---|---|---|
| Unit | `npm run test` | 326 tests, 12 files | `src/lib/` helpers — permissions, activity, utm, crypto, trend, env, finance, finance-report, salary, credit-card, password-reset, products |
| Integration | `npm run test:int` | 347 tests, 28 files | Every API route against real Postgres — auth gates, validation, persistence, audit log |
