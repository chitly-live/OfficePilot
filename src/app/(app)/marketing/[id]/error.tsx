...
/**
 * `error.tsx` for `/marketing/[id]` — fallback for runtime errors.
 * Distinct from `not-found.tsx`, which handles a missing campaign
 * via `notFound()`.
 */
...
export default function CampaignDetailError({
  ...
  useEffect(() => {
    ...
    console.error('[marketing/[id]] route error', error);
  }, [error]);
    ...
    <div className="flex flex-col items-center justify-center gap-4 rounded-lg border border-dashed bg-muted/20 px-6 py-16 text-center">
      ...
      <div className="space-y-1">
        ...
        <p className="max-w-sm text-sm text-muted-foreground">
          Something went wrong fetching the campaign record. Try
