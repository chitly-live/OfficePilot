...
/**
 * `error.tsx` for `/social` — graceful fallback when the server page
...
export default function SocialError({ error, reset }: SocialErrorProps) {
  useEffect(() => {
    ...
    <div className="flex flex-col items-center justify-center gap-4 rounded-lg border border-dashed bg-muted/20 px-6 py-16 text-center">
      ...
      <div className="space-y-1">
        <h2 className="text-base font-semibold text-foreground">
          Couldn&apos;t load social posts
        </h2>
        <p className="max-w-sm text-sm text-muted-foreground">
          Something went wrong while fetching the social calendar. Try
